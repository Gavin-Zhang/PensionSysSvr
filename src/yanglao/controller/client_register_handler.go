...
func RegisterClientHandler(w http.ResponseWriter, r *http.Request) {
	...
	ret, err := gonet.CallByName("mysqlsvr", "CheckChinaID", client.ChinaId)
	if err != nil {
		seelog.Error("RegisterClientHandler call mysqlsvr function CheckChinaID err:", err)
	...
	ret, err = gonet.CallByName("mysqlsvr", "RegisterClient", &client)
	if err != nil {
		seelog.Error("RegisterClientHandler call mysqlsvr function RegisterClinet err:", err)
		sendErr(w, constant.ResponseCode_ProgramErr, "内部程序错误")
		return
	}
