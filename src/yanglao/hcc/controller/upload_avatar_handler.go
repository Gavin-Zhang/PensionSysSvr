...
func UploadAvatarHandler(w http.ResponseWriter, r *http.Request) {
	...
	ret, err := gonet.CallByName("HccMysqlSvr", "SetAvatar", idxs[0], outname)
	if err != nil {
		seelog.Error("UploadAvatarHandler call HccMysqlSvr function SetAvatar err:", err)
		sendErr(w, constant.ResponseCode_ProgramErr, "内部程序错误")
		return
	}
	result := false
	...
	if !result {
		seelog.Error("UploadAvatarHandler HccMysqlSvr function SetAvatar update fail")
		sendErr(w, constant.ResponseCode_ProgramErr, "内部程序错误")
		return
	}

	dir := "image/hcc/avatar"
