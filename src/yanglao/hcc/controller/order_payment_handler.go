...
func OrderPaymentHandler(w http.ResponseWriter, r *http.Request) {
	...
	ret, err := gonet.CallByName("HccMysqlSvr", "GetOrder", order.Idx)
	if err != nil {
		seelog.Error("OrderPaymentHandler call HccMysqlSvr function GetOrder err:", err)
	...
	if err != nil {
		seelog.Error("OrderPaymentHandler call HccMysqlSvr function FinisOrder err:", err)
		sendErr(w, constant.ResponseCode_ProgramErr, "内部程序错误")
		return
	}
...
func DoFinish(order *structure.Order, consumptionType string) ([]interface{}, error) {
	if consumptionType == "政府购买" {
		return gonet.CallByName("HccMysqlSvr", "FinisOrder", order,
