...
func DeletePhotosHandler(w http.ResponseWriter, r *http.Request) {
	...
	if checkSession(w, r) == nil {
		seelog.Error("EES DeletePhotosHandler not find player by cookie")
	...
	if !checkNotEmptyParams(r, params) {
		seelog.Error("EES DeletePhotosHandler checkNotEmptyParams fail")
		sendErr(w, constant.ResponseCode_ParamErr, "信息不全")
		return
	}

	dir := "./image/ees/photo/" + fmt.Sprintf("%x/%s", md5.Sum([]byte(r.FormValue("clientidx"))), r.FormValue("idx"))
	file := dir + "/" + r.FormValue("name")

	if err := os.Remove(file); err != nil {
		seelog.Error("EES DeletePhotosHandler os.Remove error:", err)
